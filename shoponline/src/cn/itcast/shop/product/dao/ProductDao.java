...
public class ProductDao extends HibernateDaoSupport {
	...
	public List<Product> findNew() {
		...
		criteria.addOrder(Order.desc("pdate"));
		// 执行查询:
		List<Product> list = this.getHibernateTemplate().findByCriteria(criteria, 0, 15);
		return list;
	}
	// 根据商品的轮播图
	public List<Product> findBanner() {
		// 使用离线条件查询.
		DetachedCriteria criteria = DetachedCriteria.forClass(Product.class);
		//条件就是style = 4
		criteria.add(Restrictions.eq("style",4));
		// 倒序排序输出:
		criteria.addOrder(Order.desc("pdate"));
		// 执行查询:
		List<Product> list = this.getHibernateTemplate().findByCriteria(
	...
	// 根据图标
	public List<Product> findIcon() {
		// 使用离线条件查询.
		DetachedCriteria criteria = DetachedCriteria.forClass(Product.class);
		// 条件就是style=3
		criteria.add(Restrictions.eq("style", 3));
		// 倒序排序输出:
		criteria.addOrder(Order.desc("pdate"));
		// 执行查询:
		List<Product> list = this.getHibernateTemplate().findByCriteria(
	...
	// 根据商品的鞋类商品
	public List<Product> findFeet() {
		// 使用离线条件查询.
		DetachedCriteria criteria = DetachedCriteria.forClass(Product.class);
		// 条件就是style=1
		criteria.add(Restrictions.eq("style", 1));
		// 倒序排序输出:
		criteria.addOrder(Order.desc("pdate"));
		// 执行查询:
		List<Product> list = this.getHibernateTemplate().findByCriteria(
	...
	public List<Product> findFruit() {
		...
		DetachedCriteria criteria = DetachedCriteria.forClass(Product.class);
		// 条件就是style=2
		criteria.add(Restrictions.eq("style",2));
		// 倒序排序输出:
		criteria.addOrder(Order.desc("pdate"));
		// 执行查询:
		List<Product> list = this.getHibernateTemplate().findByCriteria(
	...
	public List<Product> findByPagePname(String pname, int begin, int limit) {
		...
		List<Product> list = this.getHibernateTemplate()
				.execute(new PageHibernateCallback<Product>(hql, new Object[]{}, begin, limit));
		if (list != null && list.size() > 0) {
			return list;
		}
		/*
