...
<HTML>
	...
	<body>
		...
		<form id="userAction_save_do" name="Form1" action="${pageContext.request.contextPath}/adminProduct_update.action" method="post" enctype="multipart/form-data">
			...
			&nbsp;
			<table cellSpacing="1" cellPadding="6" width="100%" align="center" bgColor="#eeeeee" style="border: 1px solid #8ba7e3" border="0">
				<tr>
				...
				<tr>
					...
					<td class="ta_01" bgColor="#ffffff">
						<select name="style">
							<option value="1" <s:if test="model.style==1">selected</s:if>>轮播图</option>
							<option value="2" <s:elseif test="model.style==2">selected</s:elseif>>banner图</option>
							<option value="3" <s:elseif test="model.style==3">selected</s:elseif>>banner图</option>
							<option value="4" <s:elseif test="model.style==4">selected</s:elseif>>水蔬类</option>
							<option value="4" <s:else>selected</s:else>>普通</option>
						</select>
					</td> 
				</tr>
				<tr>
				...
				<tr>
					...
					<td class="ta_01" bgColor="#ffffff" colspan="6">
						<input type="file" name="upload" />
					</td>
				</tr>
				<tr>
